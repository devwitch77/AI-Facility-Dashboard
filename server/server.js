...
const ensureUsers = async () => {
  ...
  for (const u of defaults) {
    ...
    if (existing.rows.length === 0) {
      ...
      await pool.query("INSERT INTO users (email, password_hash, role) VALUES ($1,$2,$3)", [u.email, hashed, u.role]);
...
async function ensureSchema() {
  ...
  `);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_sensor_data_name_time ON sensor_data (sensor_name, recorded_at DESC);`);
  await pool.query(`
  ...
  `);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_alerts_time ON alerts (triggered_at DESC);`);
}
async function seedSensorsIfEmpty() {
  ...
  for (const s of seeds) {
    await pool.query(
      `INSERT INTO sensors (name, value, unit, updated_at) VALUES ($1,$2,$3,NOW())
       ON CONFLICT (name) DO UPDATE SET value=EXCLUDED.value, unit=EXCLUDED.unit, updated_at=NOW();`,
      [s.name, s.value, s.unit]
    );
    await pool.query(
      `INSERT INTO sensor_data (sensor_name, value, unit, recorded_at) VALUES ($1,$2,$3,NOW());`,
      [s.name, s.value, s.unit]
    );
  }
...
function withinAlertRange(name, value){
  const thr = THRESHOLDS[name]; if (!thr) return null;
  if (value > thr.max) return "high";
  if (value < thr.min) return "low";
  return null;
}
async function getLatestSensors(){
  const { rows } = await pool.query(`SELECT id, name, value, unit, updated_at FROM sensors ORDER BY name ASC;`);
  return rows;
}

app.get("/api/sensors", async (_req, res) => {
  try { res.json(await getLatestSensors()); }
  catch (e) { console.error(e); res.status(500).json({ error: "Internal server error" }); }
});
app.get("/api/alerts", async (_req, res) => {
  try {
    const { rows } = await pool.query(
      `SELECT id, sensor_name, value, status, triggered_at FROM alerts ORDER BY triggered_at DESC LIMIT 200;`
    );
    res.json(rows);
  } catch (e) { console.error(e); res.status(500).json({ error: "Internal server error" }); }
});
app.get("/api/readings", async (req, res) => {
  ...
  try {
    ...
    `;
    const params = [];
    if (sensor) { q += ` AND sensor_name=$1`; params.push(sensor); }
...
app.post("/api/reading", async (req, res) => {
  try {
    ...
    await pool.query(`INSERT INTO sensor_data (sensor_name, value, unit, recorded_at) VALUES ($1,$2,$3,NOW());`, [name, val, unit || null]);
    ...
    if (status) {
      await pool.query(`INSERT INTO alerts (sensor_name, value, status, triggered_at) VALUES ($1,$2,$3,NOW());`, [name, val, status]);
      io.emit("sensor-alert", { sensor: name, value: val, status, time: new Date() });
    }
...
app.delete("/api/alerts/clear", verifyToken, checkRole(["admin"]), async (_req, res) => {
  try { await pool.query("DELETE FROM alerts"); io.emit("alerts-cleared"); res.json({ message: "✅ All alerts cleared." }); }
  catch (e) { console.error(e); res.status(500).json({ error: "Internal server error" }); }
});
...
io.on("connection", async (socket) => {
  ...
  try {
    ...
    const { rows: alerts } = await pool.query(
      `SELECT sensor_name, value, status, triggered_at FROM alerts ORDER BY triggered_at DESC LIMIT 50;`
    );
    socket.emit("all-alerts", alerts.map(a => ({
      sensor: a.sensor_name, value: Number(a.value), status: a.status, time: new Date(a.triggered_at),
    })));
...
async function simulateOnce() {
  for (const name of SENSOR_NAMES) {
    ...
    try {
      const { rows } = await pool.query(`SELECT value FROM sensors WHERE name=$1;`, [name]);
      if (rows.length) latest = Number(rows[0].value);
    } catch {}

    const drift = Math.random() * 2 - 1;
    let candidate = Number((latest + drift).toFixed(2));
    candidate = Math.max(thr.min - 10, Math.min(thr.max + 10, candidate));

    const unit = name.includes("Temperature") ? "°C" :
                 name.includes("Humidity")    ? "%"  :
                 name.includes("CO2")         ? "ppm":
                 name.includes("Light")       ? "lux": null;

    try {
      await pool.query(`INSERT INTO sensor_data (sensor_name, value, unit, recorded_at) VALUES ($1,$2,$3,NOW());`, [name, candidate, unit]);
      const up = await pool.query(
        `INSERT INTO sensors (name, value, unit, updated_at) VALUES ($1,$2,$3,NOW())
         ON CONFLICT (name) DO UPDATE SET value=EXCLUDED.value, unit=EXCLUDED.unit, updated_at=NOW()
         RETURNING *;`,
      ...
      if (status) {
        await pool.query(`INSERT INTO alerts (sensor_name, value, status, triggered_at) VALUES ($1,$2,$3,NOW());`, [name, candidate, status]);
        io.emit("sensor-alert", { sensor: name, value: candidate, status, time: new Date() });
      }
