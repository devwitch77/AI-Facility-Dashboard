...
/* --------------------------- DB POOL --------------------------- */
// Pure local-style config: uses DB_* only, no DATABASE_URL, no SSL tricks.
const pool = new Pool({
...
// if you serve any static files from server/public
...
/* -------------- Anti-spam alert guard parameters --------------- */
  ...
  try {
    ...
    if (status) {
      ...
      if (lastRows.length) {
        ...
        const smallDelta =
          Math.abs(Number(val) - Number(last.value)) < minDelta;
